...
/*   Updated: 2023/11/09 15:42:05 by pgrossma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_calc_mandelbrot(double a, double b, t_fractal *fractal, t_window *window)
{
	...
	a = ft_map(a, 0, window->height, -2 - (window->width / window->height / 2), 2 + (window->width / window->height / 2));
	b = ft_map(b, 0, window->width, -2 - (window->height / window->width / 2), 2 + (window->height / window->width / 2));
	fractal->ca = a;
	fractal->cb = b;
	iter = 0;
